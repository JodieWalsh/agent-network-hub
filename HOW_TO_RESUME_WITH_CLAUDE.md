...
**Update (July 3, 2026): CRM Phase 1 is COMPLETE.** The client management system now has: 5 database tables with owner-only RLS (migration `20260703010000`), a Clients list view + New Client form (`/clients`, `/clients/new`), and a full Client record page (`/clients/:id`) with Overview/Members/Tasks/Timeline tabs. Commits `46039ff`, `7b0d002`, `2147eb4`. **Next for the CRM:** Phase 1 leftovers (Kanban board view on the Clients page; stage-change controls on the Client record — stages are currently display-only), then Phase 2 (add nullable `client_id` FK to `client_briefs`, link Clients ↔ Briefs, brief summary in the record, CRM dashboard widgets). Full plan + locked decisions in `docs/CRM_ROADMAP.md` — read it before any CRM work. Subscription gating for the CRM is Dani item #23 and is NOT yet enforced.
