...
**⚠️ IMPORTANT correction to record:** the re-measurement probe **silently failed twice** — its large-font injection sometimes didn't apply, so it measured the DEFAULT font while claiming a 24px root. That produced BOTH false "artifact" readings AND false "fixed" confidence. **The committed findings doc (`185f8de`) currently mislabels forums / marketplace / Geneva dashboard as "artifacts" — that is WRONG and must be re-corrected** (forums has since been genuinely fixed, `60cc889`; the other two are real and still open). The hardened probe now logs `rootFont` and PROVES it is 24px before any number is trusted. **LESSON — from now on, verify every fix with a screenshot viewed by eye (copied to Downloads), never the probe's bare numbers alone.**

**Tomorrow's queue, in order:**
1. **Marketplace** (~427px at 24px root, REAL) — same `min-w-0`/`flex-wrap` fix pattern; verify by eye via screenshot copied to Downloads.
2. **Geneva dashboard** (~515px, REAL, the biggest) — same pattern; may have 2 culprits (region bars + attention list are the suspects).
3. **Re-correct `docs/DESIGN_REVIEW_FINDINGS.md`** with the TRUE scoreboard (forums fixed `60cc889`, marketplace fixed, Geneva dashboard fixed) and add the probe-reliability lesson.
4. Then optional: **H2** button sizes (mostly `button.tsx`, one-file win), **H4** rose small-text swaps, **C3** re-verify, then H1/H5/Mediums/Lows.

**Standing reminders:** wipe the 24 Geneva demo contacts once Dani has seen the dashboard (`node geneva-demo-data.mjs wipe`); the email logo is a someday finishing touch.

**Update (July 8, 2026, later that day): ✉️ WELCOME EMAIL SHIPPED (lead-capture piece 4) + 🏷️ OUTREACH TAG.** New waitlist signups now receive an on-brand welcome email (`b91edb4` — new `supabase/functions/_shared/waitlist-email.ts`, quiet-luxury palette with email-safe font fallbacks, wired into `geneva-lead-intake`'s new-contact branch only, try/caught so a broken email never breaks signup; **verified end-to-end with a real inbox delivery**; function live at v4). And `geneva-mailchimp-push` now adds an **"Outreach" tag** when pushing consented `interview_outreach` contacts (`21be58d` — waitlist pushes unchanged, consent wall untouched; function live at v8). Both queue items are DONE. **The outstanding queue is now:** wipe demo data after Dani reviews, questionnaire, Dani #24, mobile/large-font accessibility review.
...
5. ~~Resend welcome email~~ ✅ shipped July 8, 2026 (`b91edb4`, function live) + review system completion still open — see audit items 5–10
...
- Provider: Resend API
- FROM email: `hello@buyersagenthub.com` (✅ buyersagenthub.com is verified in Resend — confirmed in the dashboard, July 7 2026)
