...
4. **Welcome email** — on signup, send a quick friendly welcome/confirmation email via **Resend** (the app's existing email tool, from `hello@buyersagenthub.com`), respecting consent. Log a `welcome_email_sent` event to the contact's timeline. *(✅ `buyersagenthub.com` IS verified in Resend — confirmed in the dashboard July 7 2026; the app sends from `hello@buyersagenthub.com`. This piece is unblocked and ready to build.)*
