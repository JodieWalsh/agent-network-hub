...
async function createPollForPost(
  ...
  try {
    ...
    if (params.ends_at) {
      const isoDate = normaliseToISO(params.ends_at);
      console.log('[Forum] createPollForPost ends_at raw:', JSON.stringify(params.ends_at), '→ normalised:', isoDate);
      if (isoDate) pollData.ends_at = isoDate;
    }
